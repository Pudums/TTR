...
::grpc::Status TTRServer::get_player_state(::grpc::ServerContext *context,
    ...
    result->mutable_player_general_info()->set_routes_num(
        current_player.active_routes.size());
    result->mutable_player_general_info()->set_wagons_left(
        current_player.number_of_wagons_left);
    result->mutable_player_general_info()->set_current_score(
        current_player.points);
    for (const auto &route : current_player.active_routes) {
        *(result->mutable_private_info()
              ->mutable_player_routes()
              ->add_routes()) = parse_to_grpc_route(route);
    }
    for (const auto &wagon : current_player.wagon_cards) {
        *(result->mutable_private_info()
              ->mutable_player_wagons()
              ->add_wagons()) = parse_to_grpc_wagon(wagon);
    }
    return grpc::Status::OK;
}
...
::grpc::Status TTRServer::get_number_of_players(::grpc::ServerContext *context,
                                                const ::ttr::Nothing *request,
                                                ::ttr::PlayerID *response) {
